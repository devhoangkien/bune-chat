...
<template>
  <!-- 重试 -->
  <i
    v-if="status === MessageSendStatus.ERROR"
    title="重试"
    class="i-solar:refresh-linear my-a inline-block h-4.5 w-4.5 bg-theme-danger hover:rotate-180 btn-danger"
    @click="chat.retryMessage(msgId)"
  />
  <svg
    v-else-if="status === MessageSendStatus.SENDING" xmlns="http://www.w3.org/2000/svg"
    class="my-a inline-block h-4.5 w-4.5 animate-spin select-none text-small" viewBox="0 0 24 24"
