...
const showContentLoading = computed(
  () => body.value?.status !== undefined && body.value?.status === AiReplyStatusEnum.IN_PROGRESS && (data.message?.content || !body.value?.reasoningContent),
);
</script>
  ...
  <ChatMsgTemplate :prev-msg="prevMsg" :index="index" :data="data" class="group" v-bind="$attrs">
    ...
    <template #body>
      <div class="ai-reply-msg-popper relative min-h-2.5em min-w-2.6em">
        <!-- 思考内容 -->
        <p
          v-if="data?.message?.body?.reasoningContent && !isFold"
          :class="isResonFold ? 'h-2.5em truncate is-reson-open' : 'max-h-fit'"
          class="reson-content"
        ...
        >
          ...
          <span class="text absolute bottom-1.5 right-1.5 z-2 h-2em px-3 leading-2em shadow btn-info text-mini" @click="isResonFold = !isResonFold">
            {{ isResonFold ? '展开' : '收起' }}
            <i :class="isResonFold ? 'i-solar:alt-arrow-down-line-duotone' : 'i-solar:alt-arrow-up-line-duotone'" ml-1 p-1.5 />
          </span>
          <svg v-if="showReasonLoading" class="inline-block h-1.2em w-1.2em animate-spin -mb-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        />
        <svg v-if="showContentLoading" class="absolute bottom-0.75em right-0.75em h-1.2em w-1.2em animate-spin" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          <g fill="none" fill-rule="evenodd">
            <path
              d="m12.593 23.258l-.011.002l-.071.035l-.02.004l-.014-.004l-.071-.035q-.016-.005-.024.005l-.004.01l-.017.428l.005.02l.01.013l.104.074l.015.004l.012-.004l.104-.074l.012-.016l.004-.017l-.017-.427q-.004-.016-.017-.018m.265-.113l-.013.002l-.185.093l-.01.01l-.003.011l.018.43l.005.012l.008.007l.201.093q.019.005.029-.008l.004-.014l-.034-.614q-.005-.018-.02-.022m-.715.002a.02.02 0 0 0-.027.006l-.006.014l-.034.614q.001.018.017.024l.015-.002l.201-.093l.01-.008l.004-.011l.017-.43l-.003-.012l-.01-.01z"
            />
